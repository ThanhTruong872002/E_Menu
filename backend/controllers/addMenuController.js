...
class addMenuController {
    static uploadImage(req, res) {
        ...
        AddMenuModel.uploadImage(imageFile, (imageURL) => {
          ...
          res.status(200).json({ success: true, message: "Tải ảnh lên thành công", imageURL });
        });
    }

    static addDish(req, res) {
        const { menu_item_name, Description, Price, category_id, image: imageFileName } = req.body;
    
        AddMenuModel.addDish(menu_item_name, Description, Price, category_id, imageFileName, (err) => {
          if (err) {
