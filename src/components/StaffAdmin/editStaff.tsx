...
export default function EditStaff() {
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://localhost:4000/api/getAccount/${username}`)
      .then((response) => {
  ...
  const onSubmit: SubmitHandler<IEditStaffForm> = async (data) => {
    try {
      ...
      if (selectedRoleObject) {
        ...
        await axios.put(`http://localhost:4000/api/updateAccount/${username}`, {
          ...data,
          role: selectedRoleID,
        });
  ...
  return (
    <Admin>
      <div>
        <h2 className="font-[600] text-[3rem] p-8 mb-10">
