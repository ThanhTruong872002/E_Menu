  ...
  const onSubmit: SubmitHandler<IAddStaffForm> = async (data) => {
    try {
      ...
      if (!isUsernameValid) {
        ...
        setSuccessMessage(
          "Username đã bị trùng vui lòng sử dụng Username khác!"
        );
        return;
      }
      ...
      if (selectedRoleObject) {
        ...
        await axios.post("http://localhost:4000/api/addStaff", {
          ...data,
          role: selectedRoleID,
        });
  ...
  const checkUsername = async (username: string) => {
    try {
      const response = await axios.get(
        `http://localhost:4000/api/checkUsername?username=${username}`
      );
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        const selectedRoleId = roles.find(
                          (role) => role.role_name === selectedValue
                        )?.role_id;
